...
By default, the sample is configured to build with the old architecture. To run the sample with the new architecture, you can do the following:
...
```sh
Running "RtnGimbalSdkSample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
...
To edit the Objective-C or Swift files, open `sample/ios/RtnGimbalSdkSample.xcworkspace` in XCode and find the source files at `Pods > Development Pods > rtn-gimbal-sdk`.

To edit the Java or Kotlin files, open `sample/android` in Android studio and find the source files at `rtn-gimbal-sdk` under `Android`.
